 ...
 */
public class Statistic<T> implements Iterable<Data>{

    /**
     * The statistic hash map used to map the key data-element to statistic data.
     */
    private final HashMap<T, Data> mStatisticHashMap = new HashMap<T, Data>();
     ...
     * @param numberOfSets
     */
    public Statistic(int numberOfSets) {
    ...
    public void insertData(T data, int set) {
        ...
        Data dataStatistic = mStatisticHashMap.get(data);

        if (dataStatistic == null) {
            dataStatistic = new Data(data, mAmostralSizeArray.length);
    ...
    /**
     * Gets all {@link Data} mapped.
     * @return A {@link Collection} with all {@link Data}.
     */
    public Collection<Data> getStatisticalDataArray() {
     ...
     */
    public Set<Map.Entry<T, Data>> getEntrySet() {
     ...
     */
    public Data<T> getStatisticalData(T key) {
